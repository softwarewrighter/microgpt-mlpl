...
`docs/literate/microgpt-faithful.org` is a literate copy of the program (ob-mlpl). Code
...
with a detailed message -> `agentrail complete` -> push to origin. Then
report the pushed commit(s), the next step(s) from `agentrail status`, and
any blockers (e.g. pending sw-mlpl upstream fixes listed in
`docs/upstream-issues.md`).

Do not patch sw-mlpl from this repo; report gaps upstream with a minimal
reproducer.
